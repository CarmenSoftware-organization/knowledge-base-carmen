(function (C, T) { typeof exports == "object" && typeof module < "u" ? T(exports) : typeof define == "function" && define.amd ? define(["exports"], T) : (C = typeof globalThis < "u" ? globalThis : C || self, T(C.CarmenBot = {})) })(this, (function (C) {
    ...
    *:hover::-webkit-scrollbar-thumb { background: rgba(0,0,0,0.2) !important; }
    ...
    `} const v = {
        launcher: `<svg viewBox="0 0 24 24" width="32" height="32" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
    ...
    `} function O(s = { showClear: !0, showAttach: !0 }) {
        const { showClear: t, showAttach: e } = s; return `
    ...
    `} const Y = Object.freeze(Object.defineProperty({ __proto__: null, createMessageExtras: R, createRoomItemHTML: z, createTooltipHTML: A, createTypingIndicatorHTML: H, createWidgetHTML: O }, Symbol.toStringTag, { value: "Module" })); function j(s) { if (!s) return null; try { const t = /(?:youtube\.com\/(?:[^\/]+\/.+\/|(?:v|e(?:mbed)?)\/|.*[?&]v=)|youtu\.be\/)([^"&?\/\s]{11})/i, e = s.match(t); return e ? e[1] : null } catch { return null } } function U(s) { const t = /\[(.*?)\]\((https?:\/\/(?:www\.)?(?:youtube\.com|youtu\.be)[^\s<)"']+)\)/g; s = s.replace(t, (o, i, n) => { const r = j(n); return r ? `<div class="carmen-processed-video" style="margin:8px 0; border-radius:10px; overflow:hidden; position:relative; width:100%; padding-bottom:56.25%; height:0;"><iframe src="https://www.youtube.com/embed/${r}" style="position:absolute; top:0; left:0; width:100%; height:100%; border:none; border-radius:10px;" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe></div>` : o }); const e = /(https?:\/\/(?:www\.)?(?:youtube\.com|youtu\.be)[^\s<)"']+)/g; return s = s.replace(e, (o, i, n, r) => { const a = r.substring(Math.max(0, n - 10), n); if (/src=['"]$|href=['"]$|\($/.test(a) || r.substring(Math.max(0, n - 100), n).includes("carmen-processed-video")) return o; const l = j(o); return l ? `<div class="carmen-processed-video" style="margin:8px 0; border-radius:10px; overflow:hidden; position:relative; width:100%; padding-bottom:56.25%; height:0;"><iframe src="https://www.youtube.com/embed/${l}" style="position:absolute; top:0; left:0; width:100%; height:100%; border:none; border-radius:10px;" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe></div>` : o }), s } function W(s, t) { const e = o => { let i = o.trim().replace(/\\/g, "/"); if (i.includes("youtube.com") || i.includes("youtu.be") || i.startsWith("data:")) return i; const n = r => { let a = r.replace(/^https?:\/\/[^\/]+/, "").split("?")[0]; return a = a.replace(/^\/images\//, "").replace(/^\.\//, "").replace(/^\/+/, ""), a.replace(/^carmen_cloud\//, "") }; if (/^(http|https):/.test(i)) { if (!(i.includes("127.0.0.1") || i.includes("localhost") || t && i.startsWith(t))) { const a = i.split("/images/"); if (a.length > 1) return `${t}/images/${a[1]}`; const c = n(i); return `${t}/images/${c}` } return i.includes("/images/") ? i : `${t}/images/${n(i)}` } return `${t}/images/${n(i)}` }; return s = s.replace(/!\[(.*?)\]\((.*?)\)/g, (o, i, n) => { if (n.includes("youtube.com") || n.includes("youtu.be")) return o; const r = e(n); return `<br><a href="${r}" target="_blank"><img src="${r}" alt="${i}" class="carmen-processed-img"></a><br>` }), s = s.replace(/<img\s+[^>]*src=["']([^"']+)["'][^>]*>/gi, (o, i) => { if (o.includes("carmen-processed-img") || i.includes("youtube")) return o; const n = e(i); return `<br><a href="${n}" target="_blank"><img src="${n}" class="carmen-processed-img"></a><br>` }), s = s.replace(/(?:^|[\s>])(?:ดูรูป\s*)?`?((?:[\w\-\u2010\u2011\u2012\u2013]+\/)*[\w\-\u2010\u2011\u2012\u2013]+\.(?:png|jpg|jpeg|gif|svg|webp))`?/gi, (o, i) => { const r = i.replace(/[\u2010\u2011\u2012\u2013\u2014]/g, "-").replace(/^carmen_cloud\//, ""), a = `${t}/images/${r}`; return `<br><a href="${a}" target="_blank"><img src="${a}" alt="${r}" class="carmen-processed-img"></a><br>` }), s = s.replace(/<img[^>]*$/gi, ""), s } function V(s) { const t = /(https?:\/\/(?:www\.)?(?:youtube\.com|youtu\.be)[^\s<)"']+)/g; return s = s.replace(t, (e, o, i, n) => { const r = n.substring(Math.max(0, i - 10), i); return /src=['"]$|href=['"]$|>$/.test(r) ? e : `<a href="${e}" target="_blank" style="color:#2563eb; text-decoration:underline;">${e}</a>` }), s } function q(s) {
        const t = s.split(`
`); let e = [], o = !1, i = 0, n = 0; for (let r of t) { let a = r.trim(); if (/^---+$/.test(a)) { o && (e.push("</ul>"), o = !1), i = 0, n = 0, e.push('<hr style="border:none; border-top:1px solid #e2e8f0; margin:12px 0;">'); continue } if (/^### (.+)$/.test(a)) { o && (e.push("</ul>"), o = !1), i = 0, n = 0, e.push(`<div style="font-weight:700; font-size:15px; margin:12px 0 6px 0;">${a.replace(/^### /, "")}</div>`); continue } if (/^## (.+)$/.test(a)) { o && (e.push("</ul>"), o = !1), i = 0, n = 0, e.push(`<div style="font-weight:700; font-size:16px; margin:14px 0 6px 0;">${a.replace(/^## /, "")}</div>`); continue } if (/^[-*] (.+)$/.test(a)) { n = 0, o || (e.push("<ul>"), o = !0), e.push(`<li>${a.replace(/^[-*] /, "")}</li>`); continue } const c = a.match(/^(\d+)\.\s+(.+)$/); if (c) { o && (e.push("</ul>"), o = !1), i++, n = 0; const l = c[1], x = c[2]; e.push(`<div style="display:flex; gap:8px; margin:6px 0 2px 0;"><b style="min-width:20px; color:#1e40af;">${l}.</b><span>${x}</span></div>`); continue } o && !/^[-*] /.test(a) && (e.push("</ul>"), o = !1), a === "" ? (n++, n >= 2 && (i = 0), e.length > 0 && e[e.length - 1] !== "<br>" && e.push("<br>")) : (n = 0, /^\*\*.+\*\*/.test(a) && (i = 0), i > 0 ? a.startsWith("<br><a href=") || a.startsWith("<img") || a.includes("carmen-processed-video") ? e.push(`<div style="margin:2px 0 8px 28px;">${a}</div>`) : e.push(`<div style="margin:2px 0 6px 28px; color:#475569;">${a}</div>`) : e.push(a + "<br>")) } return o && e.push("</ul>"), e.join("")
    } function J(s) { return s = s.replace(/`([^`]+)`/g, '<code style="background:#f1f5f9; padding:2px 6px; border-radius:4px; font-size:13px;">$1</code>'), s = s.replace(/\*\*(.*?)\*\*/g, "<b>$1</b>"), s = s.replace(new RegExp("(?<!\\*)\\*([^*]+)\\*(?!\\*)", "g"), "<i>$1</i>"), s } function D(s, t) { if (!s) return ""; let e = String(s); const o = t ? t.replace(/\/$/, "") : ""; return e = U(e), e = W(e, o), e = V(e), e = q(e), e = e.replace(/(<br>){3,}/g, "<br><br>"), e = J(e), e } const I = { theme: "#34558b", title: "Carmen AI Specialist", showClearHistoryButton: !1, showAttachFileButton: !1, apiBase: "" }, K = ["กดปุ่ม refresh ใน workbook ไม่ได้ ทำยังไง", "บันทึกใบกำกับภาษีซื้อ ใน excel แล้ว import ได้หรือไม่", "program carmen สามารถ upload file เข้า program RDPrep ของสรรพากรได้ หรือไม่", "ฉันสามารถสร้าง ใบเสร็จรับเงิน โดยไม่เลือก Tax Invoice ได้หรือไม่", "ฉันสามารถบันทึก JV โดยที่ debit และ credit ไม่เท่ากันได้หรือไม่"], b = { error_missing_config: "❌ CarmenBot Error: กรุณาระบุ 'bu', 'user' และ 'apiBase' ใน Config ด้วยครับ (Required)", welcome_title: "สวัสดีค่ะ Carmen พร้อมช่วย!", welcome_desc: "สอบถามข้อมูลจากคู่มือบริษัท หรือเริ่มบทสนทนาใหม่ได้ทันทีด้านล่างนี้ค่ะ", history_loading: "⏳ กำลังโหลด...", alert_confirm: "ตกลง", alert_cancel: "ยกเลิก", delete_room_confirm_title: "ยืนยันลบห้องแชท?", delete_room_confirm_desc: "บทสนทนาที่เลือกจะถูกลบถาวร และไม่สามารถกู้คืนได้", clear_history_confirm_title: "ล้างประวัติห้องนี้?", clear_history_confirm_desc: "ข้อความในห้องนี้จะถูกลบทั้งหมด", file_too_large: "ไฟล์ใหญ่เกินไป", file_too_large_desc: "ไม่เกิน 5MB ครับ" }; class G {
        constructor(t) { this.bot = t, this.theme = t.theme, this.title = t.title || "Carmen AI Specialist", this.shadow = null } findElement(t) { if (!this.shadow) return document.getElementById(t) || document.querySelector(t); if (!t.includes(" ") && !t.includes(".") && !t.startsWith("#")) { const e = this.shadow.getElementById(t); return e || this.shadow.querySelector(`#${t}`) } return this.shadow.querySelector(t) } injectStyles() { if (this.shadow && this.shadow.getElementById("carmen-style")) return; const t = document.createElement("link"); t.href = "https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&family=Sarabun:wght@300;400;500;600&display=swap", t.rel = "stylesheet", document.head.appendChild(t); const e = document.createElement("style"); e.id = "carmen-style", e.innerText = F(this.theme), this.shadow ? this.shadow.appendChild(e) : document.head.appendChild(e) } createDOM() { if (document.getElementById("carmen-chat-widget")) return; const t = document.createElement("div"); t.id = "carmen-chat-widget", Object.assign(t.style, { position: "fixed", bottom: "32px", right: "32px", zIndex: "2000000", width: "0", height: "0", display: "block", pointerEvents: "none" }), document.body.appendChild(t), this.shadow = t.attachShadow({ mode: "open" }), this.shadow.innerHTML = O({ showClear: this.bot.showClearHistoryButton, showAttach: this.bot.showAttachFileButton }), this.injectStyles(); const e = this.findElement(".title-wrapper h3"); e && (e.textContent = this.title) } showWelcomeMessage() {
            const t = this.findElement("carmenChatBody"); if (!t) return; t.innerHTML = ""; const e = document.createElement("div"); e.className = "welcome-hero"; const o = b.welcome_title; e.innerHTML = `
            <div class="hero-icon">
        ...
        `, t.appendChild(e); const i = this.findElement("welcome-typing-title"), n = this.findElement("welcome-desc-text"); let r = 0; const a = () => { r < o.length ? (i.textContent += o.charAt(r), r++, setTimeout(a, 40)) : n && (n.style.opacity = "1", n.style.transform = "translateY(0)") }; setTimeout(a, 300), setTimeout(() => this.addSuggestions(), 1200)
        } showModal({ title: t, text: e, icon: o = "💡", confirmText: i = b.alert_confirm, cancelText: n = null, onConfirm: r = null }) { const a = this.findElement("carmen-alert-overlay"), c = this.findElement("carmen-alert-icon"), l = this.findElement("carmen-alert-title"), x = this.findElement("carmen-alert-desc"), g = this.findElement("carmen-alert-actions"); if (!a) return; if (c.innerHTML = o, l.textContent = t, x.innerHTML = e, g.innerHTML = "", n) { const h = document.createElement("button"); h.className = "btn-alert btn-cancel", h.textContent = n, h.onclick = () => { a.style.display = "none" }, g.appendChild(h) } const m = document.createElement("button"); m.className = "btn-alert btn-confirm", m.textContent = i, m.onclick = () => { a.style.display = "none", r && r() }, g.appendChild(m), a.style.display = "flex" } scrollToBottom() { const t = this.findElement("carmenChatBody"); t && (t.scrollTop = t.scrollHeight) } addMessage(t, e, o = null, i = null) { const n = this.findElement("carmenChatBody"); if (!n) return; const r = document.createElement("div"); r.className = `msg ${e}`; const a = D(t || "", this.bot.apiBase), c = typeof R == "function" ? R(e, o) : ""; r.innerHTML = a + c, n.appendChild(r), this.scrollToBottom(), e === "bot" && this.bot.bindCopyEvent(r) } addStreamingMessage() {
            const t = this.findElement("carmenChatBody"); if (!t) return {}; const e = document.createElement("div"); e.className = "msg bot"; const o = Date.now(); return e.innerHTML = `
        ...
        `, t.appendChild(e), this.scrollToBottom(), { container: e, loader: e.querySelector(`#loading-${o}`), span: e.querySelector(`#streaming-content-${o}`) }
        } addSuggestions(t) { this.shadow && this.shadow.querySelectorAll(".suggestions-container").forEach(n => n.remove()); const e = t || this.bot.suggestedQuestions; if (!e || e.length === 0) return; const o = this.findElement("carmenChatBody"), i = document.createElement("div"); i.className = "suggestions-container", e.forEach((n, r) => { const a = document.createElement("button"); a.innerText = n, a.className = "suggestion-chip", a.style.animationDelay = `${r * .1}s`, a.onclick = () => { this.bot.sendMessage(n), i.remove() }, i.appendChild(a) }), o.appendChild(i), this.scrollToBottom() } clearImageSelection() { this.bot.currentImageBase64 = null; const t = this.findElement("carmen-file-input"); t && (t.value = ""); const e = this.findElement("carmenImagePreview"); e && (e.style.display = "none") } renderRoomList(t, e) { const o = this.findElement("carmenRoomList"); o && (t && t.length > 0 ? (o.innerHTML = "", t.forEach(i => { const n = i.room_id === e, r = document.createElement("div"); r.innerHTML = z(i, n), o.appendChild(r.firstElementChild) })) : o.innerHTML = '<div style="padding:16px; text-align:center; color:#64748b; font-size:13px;">ไม่มีประวัติการสนทนา</div>') } showTooltip() { if (localStorage.getItem(`carmen_tooltip_seen_${this.bot.bu}`)) return; const e = document.getElementById("carmen-chat-widget"), o = document.createElement("div"); o.className = "chat-tooltip", o.id = "carmen-tooltip", o.innerHTML = A(), this.shadow ? this.shadow.appendChild(o) : e.appendChild(o), setTimeout(() => o.classList.add("show"), 2e3); const i = this.findElement("carmen-tooltip-close"), n = this.findElement("carmen-launcher"), r = () => { o.classList.remove("show"), setTimeout(() => o.remove(), 500), localStorage.setItem(`carmen_tooltip_seen_${this.bot.bu}`, "true") }; i && (i.onclick = a => { a.stopPropagation(), r() }), o && (o.onclick = () => { n && n.click(), r() }), n && n.addEventListener("click", r) } setupGlobalFunctions() { window.carmenRate = async (t, e, o) => { const i = o.parentElement; i.innerHTML = e === 1 ? '<span style="font-size:11px; color:#16a34a;">ขอบคุณค่ะ ❤️</span>' : '<span style="font-size:11px; color:#991b1b;">ขอบคุณค่ะ 🙏</span>', await this.bot.api.sendFeedback(t, e) } } bindCopyEvent(t) { const e = t.querySelector(".copy-btn"); if (!e) return; e.onclick = () => { const i = t.querySelector(".msg-bubble span") || t.querySelector(".msg-bubble") || t, n = i.innerText || i.textContent, r = () => { const a = e.innerHTML; e.innerHTML = v.check, setTimeout(() => { e.innerHTML = a }, 2e3) }; navigator.clipboard && navigator.clipboard.writeText ? navigator.clipboard.writeText(n).then(r).catch(() => o(n, r)) : o(n, r) }; function o(i, n) { const r = document.createElement("textarea"); r.value = i, r.style.cssText = "position:fixed;left:-9999px", document.body.appendChild(r), r.select(); try { document.execCommand("copy"), n() } catch (a) { console.error("Fallback copy failed", a) } document.body.removeChild(r) } } showLauncher() { const t = this.findElement("carmen-launcher"); t && (t.style.display = "flex") }
    } class X {
        constructor(t) { this.bot = t, this.api = t.api, this.roomKey = "carmen_current_room", this.typingBuffer = "", this.isTyping = !1 } async createNewChat() { this.bot.currentRoomId && await this.api.clearHistory(this.bot.currentRoomId), this.bot.currentRoomId = null, localStorage.removeItem(this.roomKey), this.bot.ui.showWelcomeMessage(), await this.loadRoomList() } async switchRoom(t) { this.bot.currentRoomId !== t && (this.bot.currentRoomId = t, localStorage.setItem(this.roomKey, t), await this.loadHistory(t), await this.loadRoomList()) } async deleteChatRoom(t) { this.bot.ui.showModal({ icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path><line x1="10" y1="11" x2="10" y2="17"></line><line x1="14" y1="11" x2="14" y2="17"></line></svg>', title: b.delete_room_confirm_title, text: b.delete_room_confirm_desc, confirmText: "ลบทิ้ง", cancelText: b.alert_cancel, onConfirm: async () => { try { await this.api.deleteRoom(t), this.bot.currentRoomId === t ? this.createNewChat() : await this.loadRoomList() } catch (e) { console.error("Delete Error:", e) } } }) } async loadRoomList() { try { const t = await this.api.getRooms(); this.bot.ui.renderRoomList(t, this.bot.currentRoomId) } catch (t) { console.error("Room List Error:", t) } } async loadHistory(t) { const e = this.bot.ui.findElement("carmenChatBody"); if (e) { e.children.length === 0 && (e.innerHTML = `<div style="text-align:center; padding:20px; color:#94a3b8;">${b.history_loading}</div>`); try { const o = await this.api.getRoomHistory(t); e.innerHTML = "", o.messages && o.messages.length > 0 ? (o.messages.forEach(i => { this.bot.ui.addMessage(i.message, i.sender, i.id, i.sources) }), setTimeout(() => this.bot.ui.scrollToBottom(), 100)) : this.bot.ui.showWelcomeMessage() } catch (o) { console.warn("History Load Error:", o), this.createNewChat() } } } async sendMessage(t = null) {
            const e = this.bot.ui.findElement("carmenUserInput"), o = t || e.value.trim(); if (!o && !this.bot.currentImageBase64) return; if (!this.bot.currentRoomId) try { const p = o.substring(0, 30) + (o.length > 30 ? "..." : ""), d = await this.api.createRoom(this.bot.bu, this.bot.username, p); this.bot.currentRoomId = d.room_id, localStorage.setItem(this.roomKey, this.bot.currentRoomId), await this.loadRoomList() } catch { this.bot.ui.addMessage("⚠️ สร้างห้องสนทนาไม่สำเร็จ", "bot"); return } let i = o; this.bot.currentImageBase64 && (i = `<img src="${this.bot.currentImageBase64}" style="max-width:100%; border-radius:8px; margin-bottom:5px;"><br>${o}`), this.bot.ui.addMessage(i, "user"), await this.api.saveMessage(this.bot.currentRoomId, { sender: "user", message: o, image: !!this.bot.currentImageBase64, timestamp: new Date().toISOString() }), e.value = ""; const n = this.bot.currentImageBase64; this.bot.ui.clearImageSelection(), this.bot.ui.shadow && this.bot.ui.shadow.querySelectorAll(".suggestions-container").forEach(p => p.remove()); const r = this.bot.ui.addStreamingMessage(); r.loader && (r.loader.style.display = "none"); const a = H(), c = document.createElement("div"); c.innerHTML = a; const l = c.firstElementChild; r.container.appendChild(l), this.bot.ui.scrollToBottom(); const x = [{ delay: 2e3, text: "กำลังค้นหาเอกสารที่เกี่ยวข้อง" }, { delay: 8e3, text: "กำลังวิเคราะห์ข้อมูล" }, { delay: 2e4, text: "กำลังเรียบเรียงคำตอบ" }, { delay: 45e3, text: "ใกล้เสร็จแล้ว กรุณารอสักครู่" }], g = [], m = l?.querySelector(".typing-status-text"); for (const p of x) g.push(setTimeout(() => { m && l.parentNode && (m.textContent = p.text, m.style.animation = "none", m.offsetHeight, m.style.animation = "") }, p.delay)); let h = "", y = null, f = null; try {
                const p = await this.api.getRoomHistory(this.bot.currentRoomId), d = p ? p.messages : [], w = (await fetch(`${this.bot.apiBase}/api/chat/stream`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ text: o, image: n, bu: this.bot.bu, username: this.bot.username, room_id: this.bot.currentRoomId, prompt_extend: this.bot.prompt_extend, history: d }) })).body.getReader(), E = new TextDecoder("utf-8"); this.typingBuffer = ""; let _ = "", M = !0; const $ = () => { if (this.typingBuffer.length > 0) { M && (g.forEach(L => clearTimeout(L)), l && l.remove(), r.span && (r.span.style.display = "block"), M = !1); const S = this.typingBuffer.length > 50 ? 5 : this.typingBuffer.length > 10 ? 2 : 1; _ += this.typingBuffer.substring(0, S), this.typingBuffer = this.typingBuffer.substring(S), r.span && (r.span.innerHTML = D(_, this.bot.apiBase), this.bot.ui.scrollToBottom()) } (this.isTyping || this.typingBuffer.length > 0) && requestAnimationFrame(() => setTimeout($, 15)) }; for (this.isTyping = !0, $(); ;) {
                    const { done: S, value: L } = await w.read(); if (S) break; const Z = E.decode(L, { stream: !0 }).split(`
`); for (const N of Z) if (N.trim()) try { const k = JSON.parse(N); if (k.type === "chunk") this.typingBuffer += k.data, h += k.data; else if (k.type === "status") { const P = l?.querySelector(".typing-status-text"); P && (P.textContent = k.data) } else k.type === "sources" ? y = k.data : k.type === "done" && (f = k.id, this.loadRoomList()) } catch { }
                } if (this.isTyping = !1, r.loader && r.loader.remove(), f && r.container) { const { createMessageExtras: S } = await Promise.resolve().then(() => Y), L = S("bot", f, y); r.container.insertAdjacentHTML("beforeend", L), this.bot.bindCopyEvent(r.container), await this.api.saveMessage(this.bot.currentRoomId, { id: f, sender: "bot", message: h, sources: y, timestamp: new Date().toISOString() }) }
            } catch (p) { console.error("Stream Error:", p) }
        }
    } class Q { constructor(t) { this.bot = t, this.closeTimeout = null } attach() { const t = this.bot.ui.findElement("carmenChatWindow"), e = this.bot.ui.findElement("carmen-launcher"), o = this.bot.ui.findElement("carmen-close-btn"), i = this.bot.ui.findElement("carmen-expand-btn"), n = this.bot.ui.findElement("new-chat-btn"), r = this.bot.ui.findElement("carmenRoomList"), a = this.bot.ui.findElement("carmen-room-dropdown-btn"), c = this.bot.ui.findElement("carmenRoomDropdownMenu"), l = this.bot.ui.findElement(".chat-header"); this.bot.ui.shadow && this.bot.ui.shadow.addEventListener("click", p => { c && c.classList.contains("show") && (p.target.closest("#carmenRoomDropdownContainer") || c.classList.remove("show")) }); const x = () => { t.classList.contains("closing") || (t.classList.add("closing"), c && c.classList.remove("show"), localStorage.setItem(`carmen_open_${this.bot.bu}`, "false"), localStorage.setItem(`carmen_expanded_${this.bot.bu}`, "false"), this.closeTimeout = setTimeout(() => { t.classList.remove("open", "expanded", "closing"), this.closeTimeout = null }, 800)) }; if (e && (e.onclick = () => { const p = t.classList.contains("open"), d = t.classList.contains("closing"); p && !d ? x() : (this.closeTimeout && (clearTimeout(this.closeTimeout), this.closeTimeout = null), t.classList.remove("closing"), t.classList.add("open"), localStorage.setItem(`carmen_open_${this.bot.bu}`, "true"), c && c.classList.remove("show"), setTimeout(() => this.bot.ui.scrollToBottom(), 100)) }), l && t) { const p = localStorage.getItem(`carmen_chat_pos_${this.bot.bu}`); if (p) try { const d = JSON.parse(p); t.style.bottom = d.bottom, t.style.right = d.right } catch { } this._setupDraggable(l, t) } o && (o.onclick = x), i && (i.onclick = () => { const p = !t.classList.contains("expanded"); if (t.classList.add("resizing"), t.classList.toggle("expanded"), localStorage.setItem(`carmen_expanded_${this.bot.bu}`, p ? "true" : "false"), p) t.style.bottom = "", t.style.right = ""; else { const d = localStorage.getItem(`carmen_chat_pos_${this.bot.bu}`); if (d) try { const u = JSON.parse(d); t.style.bottom = u.bottom, t.style.right = u.right } catch { } } !p && c && c.classList.remove("show"), setTimeout(() => { t.classList.remove("resizing"), this.bot.ui.scrollToBottom() }, 600) }), n && (n.onclick = () => { c && c.classList.remove("show"), this.bot.chat.createNewChat() }), a && c && (a.onclick = p => { p.stopPropagation(), c.classList.toggle("show") }), r && (r.onclick = async p => { const d = p.target.closest(".delete-room-btn"), u = p.target.closest(".room-dropdown-item"); if (d) { p.stopPropagation(); const w = d.getAttribute("data-id"); this.bot.chat.deleteChatRoom(w); return } if (u) { const w = u.getAttribute("data-id"); this.bot.chat.switchRoom(w), c && c.classList.remove("show") } }, r.addEventListener("wheel", p => { const d = p.deltaY > 0, u = p.deltaY < 0, w = r.scrollTop + r.clientHeight >= r.scrollHeight - 1, E = r.scrollTop <= 0; (d && w || u && E) && p.preventDefault(), p.stopPropagation() }, { passive: !1 })); const g = this.bot.ui.findElement("carmen-clear-btn"); g && (g.onclick = () => { this.bot.currentRoomId && this.bot.ui.showModal({ icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path><line x1="10" y1="11" x2="10" y2="17"></line><line x1="14" y1="11" x2="14" y2="17"></line></svg>', title: b.clear_history_confirm_title, text: b.clear_history_confirm_desc, confirmText: "ลบเลย", cancelText: b.alert_cancel, onConfirm: async () => { await this.bot.api.clearHistory(this.bot.bu, this.bot.username, this.bot.currentRoomId), await this.bot.chat.loadHistory(this.bot.currentRoomId) } }) }); const m = this.bot.ui.findElement("carmen-attach-btn"); if (m) { const p = this.bot.ui.findElement("carmen-file-input"); m.onclick = () => p.click(), p.onchange = d => { const u = d.target.files[0]; if (!u) return; if (u.size > 5 * 1024 * 1024) { this.bot.ui.showModal({ icon: "⚠️", title: b.file_too_large, text: b.file_too_large_desc }), p.value = ""; return } const w = new FileReader; w.onload = E => { this.bot.currentImageBase64 = E.target.result; const _ = this.bot.ui.findElement("carmenImagePreview"), M = this.bot.ui.findElement("preview-img-element"); _ && M && (M.src = this.bot.currentImageBase64, _.style.display = "flex") }, w.readAsDataURL(u) } } const h = this.bot.ui.findElement("clear-image-btn"); h && (h.onclick = () => this.bot.ui.clearImageSelection()); const y = this.bot.ui.findElement("carmen-send-btn"), f = this.bot.ui.findElement("carmenUserInput"); y && (y.onclick = () => { this.bot.chat.sendMessage(), f && (f.style.height = "auto") }), f && (f.addEventListener("input", function () { this.style.height = "auto", this.style.height = this.scrollHeight + "px" }), f.addEventListener("keydown", p => { p.key === "Enter" && !p.shiftKey && (p.preventDefault(), this.bot.chat.sendMessage(), f.style.height = "auto") })) } _setupDraggable(t, e) { let o = 0, i = 0, n = 0, r = 0, a = !1, c = null; const l = m => { if (e.classList.contains("expanded")) return; const h = m.type.startsWith("touch") ? m.touches[0] : m; o = h.clientX, i = h.clientY; const y = window.getComputedStyle(e); n = parseInt(y.bottom) || 84, r = parseInt(y.right) || 0, window.addEventListener("mousemove", x, { passive: !1 }), window.addEventListener("mouseup", g), window.addEventListener("touchmove", x, { passive: !1 }), window.addEventListener("touchend", g), a = !1, e.style.transition = "none", document.body.style.userSelect = "none", document.body.style.webkitUserSelect = "none" }, x = m => { const h = m.type.startsWith("touch") ? m.touches[0] : m, y = h.clientX, f = h.clientY; !a && Math.abs(y - o) < 5 && Math.abs(f - i) < 5 || (a || (a = !0, e.style.top = "auto", e.style.left = "auto"), m.cancelable && m.preventDefault(), c && cancelAnimationFrame(c), c = requestAnimationFrame(() => { const p = o - y, d = i - f, u = n + d, w = r + p, E = e.getBoundingClientRect(), _ = -22, M = window.innerHeight - E.height - 42, $ = -22, S = window.innerWidth - E.width - 42; e.style.bottom = Math.min(Math.max(_, u), M) + "px", e.style.right = Math.min(Math.max($, w), S) + "px" })) }, g = () => { window.removeEventListener("mousemove", x), window.removeEventListener("mouseup", g), window.removeEventListener("touchmove", x), window.removeEventListener("touchend", g), c && cancelAnimationFrame(c), e.style.transition = "", document.body.style.userSelect = "", document.body.style.webkitUserSelect = "", a && localStorage.setItem(`carmen_chat_pos_${this.bot.bu}`, JSON.stringify({ bottom: e.style.bottom, right: e.style.right })) }; t.addEventListener("mousedown", l), t.addEventListener("touchstart", l, { passive: !0 }), t.style.cursor = "move" } } class B { constructor(t = {}) { if (Object.keys(t).length === 0 && (t = this._getConfigFromScript()), !t.bu || !t.user || !t.apiBase) { console.error(b.error_missing_config), t.isCustomElement || alert(b.error_missing_config); return } this.apiBase = (t.apiBase || I.apiBase).replace(/\/$/, ""), this.bu = t.bu, this.username = t.user, this.theme = t.theme || I.theme, this.title = t.title || I.title, this.prompt_extend = t.prompt_extend || null, this.showClearHistoryButton = t.showClearHistoryButton ?? I.showClearHistoryButton, this.showAttachFileButton = t.showAttachFileButton ?? I.showAttachFileButton, this.suggestedQuestions = K, this.currentImageBase64 = null, this.currentRoomId = null, localStorage.removeItem(`carmen_room_${this.bu}_${this.username}`), this.api = new T(this.apiBase), this.ui = new G(this), this.chat = new X(this), this.events = new Q(this), this.init() } _getConfigFromScript() { const t = document.currentScript || document.querySelector('script[src*="carmen-widget.js"]'); return t ? { bu: t.getAttribute("data-bu"), user: t.getAttribute("data-user"), apiBase: t.getAttribute("data-api-base"), theme: t.getAttribute("data-theme"), title: t.getAttribute("data-title"), prompt_extend: t.getAttribute("data-prompt-extend") } : {} } async init() { this.ui.injectStyles(), this.ui.createDOM(), this.ui.setupGlobalFunctions(), this.events.attach(), this.ui.showLauncher(), await this.chat.loadRoomList(), this.ui.showWelcomeMessage(), this.ui.showTooltip(), this._restoreUIState() } _restoreUIState() { const t = localStorage.getItem(`carmen_open_${this.bu}`) === "true", e = localStorage.getItem(`carmen_expanded_${this.bu}`) === "true"; if (t) { const o = this.ui.findElement("carmenChatWindow"); o && o.classList.add("open") } if (e) { const o = this.ui.findElement("carmenChatWindow"); if (o) { o.classList.add("expanded"); const i = this.ui.findElement("carmenSidebar"); i && i.classList.add("sidebar-visible") } } } sendMessage(t) { return this.chat.sendMessage(t) } bindCopyEvent(t) { this.ui.bindCopyEvent(t) } } if (window.CarmenBot = B, "customElements" in window) { class s extends HTMLElement { connectedCallback() { const e = this.getAttribute("bu"), o = this.getAttribute("user"), i = this.getAttribute("api-base"); e && o && i && new B({ bu: e, user: o, apiBase: i, theme: this.getAttribute("theme"), title: this.getAttribute("title"), isCustomElement: !0 }) } } customElements.define("carmen-chatbot", s) } document.currentScript && document.currentScript.hasAttribute("data-bu") && new B, C.CarmenBot = B, Object.defineProperty(C, Symbol.toStringTag, { value: "Module" })
}));

...
- **Run migrations with `psql`, not `./server migrate`.** The Go migrate splits on `;` and corrupts files with `DO $$ … $$` / PL/pgSQL (e.g. `0001_init_schema.sql`). Use `scripts/migrate-docker.sh`. There is a single canonical migration `0001_init_schema.sql` (idempotent); see `backend/migrations/README.md`.
